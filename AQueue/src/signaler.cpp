...
#define AQUEUE_SIGNALER_WAIT_BASED_ON_SELECT
#elif defined AQUEUE_FORCE_POLL
#define AQUEUE_SIGNALER_WAIT_BASED_ON_POLL
...
#if defined AQUEUE_SIGNALER_WAIT_BASED_ON_POLL
#include <poll.h>
#elif defined AQUEUE_SIGNALER_WAIT_BASED_ON_SELECT
...
#if defined AQUEUE_HAVE_EVENTFD
...
static int sleep_ms (unsigned int ms_)
{
    ...
    return 0;
#elif defined AQUEUE_HAVE_ANDROID
    usleep (ms_ * 1000);
    return 0;
#else
...
aqueue::fd_t aqueue::signaler_t::get_fd ()
{
    return r;
}

void aqueue::signaler_t::send ()
{
#if HAVE_FORK
    ...
    aqueue_assert (nbytes == sizeof (dummy));
#else
    unsigned char dummy = 0;
    while (true) {
...
int aqueue::signaler_t::wait (int timeout_)
{
#ifdef HAVE_FORK
    if (unlikely(pid != getpid()))
    {
...
#ifdef AQUEUE_SIGNALER_WAIT_BASED_ON_POLL
...
#endif
    aqueue_assert (rc == 1);
    aqueue_assert (pfd.revents & POLLIN);
    return 0;
...
#endif
    aqueue_assert (nbytes == sizeof (dummy));
    aqueue_assert (dummy == 0);
#endif
}

#ifdef HAVE_FORK
void aqueue::signaler_t::forked()
{
...
int aqueue::signaler_t::make_fdpair (fd_t *r_, fd_t *w_)
{
#if defined AQUEUE_HAVE_EVENTFD
	fd_t fd = eventfd (0, 0);
	if (fd == -1) {
...
#   endif
    ...
    *w_ = open_socket (AF_INET, SOCK_STREAM, 0);
    ...
    aqueue_assert (dwrc == WAIT_OBJECT_0);
...
#if defined AQUEUE_SIGNALER_WAIT_BASED_ON_SELECT
#undef AQUEUE_SIGNALER_WAIT_BASED_ON_SELECT
#endif
#if defined AQUEUE_SIGNALER_WAIT_BASED_ON_POLL
#undef AQUEUE_SIGNALER_WAIT_BASED_ON_POLL
#endif
